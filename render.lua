...
function M.draw(game, layout, hover_row, hover_col)
	local lg = love.graphics
	lg.clear(config.COLOR_BOARD[1], config.COLOR_BOARD[2], config.COLOR_BOARD[3])
	M._draw_grid(layout)
	M._draw_stones(game.board, layout)
	if hover_row and hover_col and not game.over and game.to_play == config.HUMAN_COLOR then
	...
	local footer = string.format(
		"Prisoners — Black: %d  White: %d  —  P pass  R restart  Esc quit",
		pb,
		pw
	)
