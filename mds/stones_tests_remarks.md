...
Those tests are mostly uselsess, there is complete lack of understanding of how how this stone should work, we should have multiple controlled territory (new parameter of game state that for each field has an assigned number therefore we need a test_helper function which sets it up using ASCII) then we need to assert a score after stone placed in different parts of the board depending if its white or black player. Read enclosure rules, Read enclosure rules, There should be multiple enclosures on board, tests with enclosures within enclosures, opponents enclosure within our enclosure, enclosures using the boundaries of the board etc.
