...
--- @return table
function M.layout(anchor, title, description, screen_w, screen_h)
	...
	local desc_h = wrapped_text_height(description, desc_w, desc_font) + INNER_PAD * 2
	...
	desc_h = wrapped_text_height(description, desc_w_actual, desc_font) + INNER_PAD * 2
	desc_h = math.max(52, desc_h)
...
function M.draw(box)
	...
	lg.printf(box.description, db.x + INNER_PAD, db.y + INNER_PAD, box.desc_text_w, "left")
