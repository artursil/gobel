...
describe("T-050 stone metadata and behavior hooks", function()
	...
	it("effects array contains valid effect entries for each stone", function()
		...
		for _, stone in pairs(content.stones) do
			...
			for i = 1, #stone.effects do
				...
				assert.is_true(effect.action ~= nil and effect.phase ~= nil)
			end
		end
	end)
	...
	assert.is_true(resolver.begin_turn(state, "black").ok)
		...
		assert.are.equal(mult_before + P.stone_plus_mult("stone_focus"), black.score.plus_mult)
		assert.are.equal(P.stone_placement_message("stone_focus"), state.messages.recent[#state.messages.recent])
	end)
